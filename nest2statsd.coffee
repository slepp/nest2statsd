...
ref.on 'value', (snapshot) ->
  ...
  for id, structure of data.structures
    structureName = structure.name?.replace(/[^A-Za-z0-9]+/g, '_').replace(/_$/, '') ? 'structure'
    ...
    for id in structure.thermostats
      ...
      metricName = (config.prefix ? 'nest-api') + "." + structureName + "." + thermo.name.replace(/[^A-Za-z0-9]+/g, '_').replace(/_$/, '')
